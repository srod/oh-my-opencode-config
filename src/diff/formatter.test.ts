...
function stripAnsi(str: string): string {
  // biome-ignore lint/suspicious/noControlCharactersInRegex: Required to strip ANSI escape codes from chalk output
...
describe("formatter", () => {
  ...
  test("formatDiff returns human readable string", () => {
    const output = stripAnsi(formatDiff(entries))
